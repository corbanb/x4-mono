...
describe('ProjectSchema', () => {
  ...
  test('rejects name over 255 chars', () => {
    expect(ProjectSchema.safeParse({ ...validProject, name: 'a'.repeat(256) }).success).toBe(false);
  });
...
describe('UpdateProjectSchema', () => {
  ...
  test('accepts input with id and status only', () => {
    expect(UpdateProjectSchema.safeParse({ id: VALID_UUID, status: 'archived' }).success).toBe(
      true,
    );
  });
  ...
  test('rejects name over 255 chars when provided', () => {
    expect(UpdateProjectSchema.safeParse({ id: VALID_UUID, name: 'a'.repeat(256) }).success).toBe(
      false,
    );
  });
  ...
  test('rejects invalid status when provided', () => {
    expect(UpdateProjectSchema.safeParse({ id: VALID_UUID, status: 'deleted' }).success).toBe(
      false,
    );
  });
});
