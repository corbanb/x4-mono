...
describe('ai.generate', () => {
  ...
  test('rejects unauthenticated request', async () => {
    ...
    await expect(caller.ai.generate({ prompt: 'Hello' })).rejects.toThrow();
  });

  test('rejects empty prompt', async () => {
    const caller = createCaller(createTestContext());

    await expect(caller.ai.generate({ prompt: '' })).rejects.toThrow();
  });

  test('rejects prompt exceeding max length', async () => {
    const caller = createCaller(createTestContext());

    await expect(caller.ai.generate({ prompt: 'x'.repeat(10001) })).rejects.toThrow();
  });

  test('rejects maxTokens exceeding limit', async () => {
    const caller = createCaller(createTestContext());

    await expect(caller.ai.generate({ prompt: 'Hello', maxTokens: 5000 })).rejects.toThrow();
  });
