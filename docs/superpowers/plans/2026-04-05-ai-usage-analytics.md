...
**Tech Stack:** Bun, tRPC v11, Drizzle ORM (Neon Postgres), Next.js 15 App Router, recharts 2.15.4, Zod 3, `@x4/shared/types`, `lucide-react`
...
```bash
cd /path/to/repo && bun turbo type-check --filter=@x4/shared
```
...
```bash
bun turbo type-check --filter=@x4/api
```
