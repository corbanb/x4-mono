...
const mockGigetDownload = mock(async (_source: string, _opts: { dir: string; force: boolean }) => {});
...
function setupGigetMock() {
  mockGigetDownload.mockImplementation(async (_source: string, opts: { dir: string }) => {
    ...
    writeFileSync(join(dir, "turbo.json"), JSON.stringify({ globalEnv: ["DATABASE_URL"], tasks: { build: {} } }));
    ...
    writeFileSync(join(dir, "apps/api/package.json"), JSON.stringify({ name: "@x4/api", dependencies: { "@x4/shared": "workspace:*" } }));
    ...
    writeFileSync(join(dir, "apps/web/package.json"), JSON.stringify({ name: "@x4/web", dependencies: { "@x4/shared": "workspace:*" } }));
    // Shared
    mkdirSync(join(dir, "packages/shared"), { recursive: true });
    writeFileSync(join(dir, "packages/shared/package.json"), JSON.stringify({ name: "@x4/shared" }));
  });
}
