...
describe("createNeonProject", () => {
    ...
    globalThis.fetch = mock(async () => {
      return new Response(
        JSON.stringify({
          ...
          connection_uris: [{ connection_uri: "postgresql://neon:pass@ep-cool-123.eu-central-1.aws.neon.tech/neondb" }],
        }),
        { status: 200 },
      );
    }) as typeof fetch;
  ...
  test("throws on non-OK response with status and body", async () => {
    ...
    await expect(createNeonProject("bad-key", "test")).rejects.toThrow(
      /Neon API error \(401\)/,
    );
  });
