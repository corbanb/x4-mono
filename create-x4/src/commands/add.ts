    ...
    for (const appDir of appDirs) {
      ...
      if (existsSync(appJsonPath)) {
        try {
          ...
          const iosBundleId = appJson.expo?.ios?.bundleIdentifier as
            | string
            | undefined;
          if (iosBundleId) {
...
export async function runAddCommand(rawArgs: string[]): Promise<void> {
  ...
  const targetDirName =
    templateType === "mobile-app" ? `mobile-${name}` : name;
  ...
  if (templateType === "mobile-app") {
    ...
    const mobileNameClean = name.charAt(0).toUpperCase() + name.slice(1).replace(/-([a-z])/g, (_, c: string) => c.toUpperCase());
